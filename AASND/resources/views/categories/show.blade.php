...
@section('header')
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <h1 style="font-size: 1.875rem; font-weight: bold; color: #111827;">{{ $category->name }}</h1>
        @auth
            @if(auth()->user()->isAdmin())
                <div style="display: flex; gap: 0.5rem;">
                    <a href="{{ route('admin.categories.edit', $category) }}" class="btn btn-warning" style="padding: 0.5rem 1rem; text-decoration: none;">
                        Edit
                    </a>
...
@if($category->description)
    <p style="color: #4b5563; margin-bottom: 2rem;">{{ $category->description }}</p>
@endif
